...
% Option for fishing with fishery squeeze at time of MPA 
...
% Variable variables -------------------------------------    
    ...
    % 1,1 is NOT declining (baseline)
    Dec = linspace(1, 1, tInit+tPost);
    % adding a 50% decline over 50 years.
    Dec(end-49:end) = linspace(1, 0.5, 50);
    % adding a 50% increase over 50 years
    % Dec(end-49:end) = linspace(1, 1.5, 50);
...
% Noise -------------------------------
    ...
    % pinknoise (requires Audio Toolbox)
...
% Model set-up -----------------------------    
    ...
    % Model Function:
    DD_func =  str2func('Func_ProjPop_2pop_BHv2'); 
...
% Pre-reserv (intial conditions)-----       
    ...
    [NPre, R] = DD_func('fished', OC, SpParas, Noise, ...
                        Dec(1:tInit), FSq, ...
...
% Post-reserve ----
    [NPost1, ~] = DD_func('reserve', OC, SpParas, Noise, ...
                        Dec((tInit+1):end), FSq, ...
                        NPre(:,end,:,:),...
...
%     plot(1:(tInit+tPost), [squeeze(sum(NPre,1));squeeze(sum(NPost,1))])
 ...
 % Sample the popualtions (i.e. include measurement error )
    % affects all years post reserves 
    % k = meaure of dispersion, small = more clumping, lrg = more dispersed
    % estimated from PISCO data (see Hopf et al 2022 Biological Conservation)
    k = 1; 
    % sample with neg bionomial
    NPost = nbinrnd(k, k/(NPost1 + k));  
    
    % checking how many replicate timeseries have non-zero values
    % (for each population)
    % (we are disgarding zeros to match data analysis, but still want 2000 reps)
    sum(sum(sum(NPost(Rind(1):Rind(2),:,:)) == 0) == 0)
    sum(sum(sum(NPost(Find(1):Find(2),:,:)) == 0) == 0)


   % remove timseries with at least one zero and get 2000 reps
   % (get times series that dont have a year with 0)
    NPost = NPost(:,:, ...
          find(sum(sum(NPost(Rind(1):Rind(2),:,:)) == 0) == 0 & sum(sum(NPost(Find(1):Find(2),:,:)) == 0) == 0));
    NPost = NPost(:,:,1:2000);   
...
% calculate biomass
    ...
    NbioPost = NPost.*repmat(Weights,p,tPost,2000);
...
% Save csv
% writetable(Outputs, datestr(now, 'yyyy-mm-dd') + "_" + OC + "_FisherySqueeze_" + FSq + "_NoiseCorr_" + Noise{2} + "_decline_" + Dec(end) + "_V0results_5pn2000_MError_BR.csv")
